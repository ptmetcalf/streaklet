...
def test_no_streak_on_empty_db(test_db: Session):
    """Test that streak is 0 when no days are completed."""
    streak_count, last_date = streak_service.calculate_current_streak(test_db)
    assert streak_count == 0
    assert last_date is None


def test_single_day_streak(test_db: Session):
    """Test streak of 1 for a single completed day."""
    today = date.today()
    status = DailyStatus(date=today, completed_at=datetime.now())
    test_db.add(status)
    test_db.commit()
...
def test_consecutive_days_streak(test_db: Session):
    """Test streak increments for consecutive days."""
    today = date.today()

    for i in range(5):
        day = today - timedelta(days=i)
        status = DailyStatus(date=day, completed_at=datetime.now())
        test_db.add(status)
    test_db.commit()
...
def test_streak_resets_on_missed_day(test_db: Session):
    """Test streak resets when a day is missed."""
    today = date.today()

    status1 = DailyStatus(date=today, completed_at=datetime.now())
    status2 = DailyStatus(date=today - timedelta(days=1), completed_at=datetime.now())
    test_db.add(status1)
    test_db.add(status2)
    test_db.commit()
    ...
    status3 = DailyStatus(date=today - timedelta(days=4), completed_at=datetime.now())
    test_db.add(status3)
    test_db.commit()
...
def test_get_streak_info(test_db: Session, sample_tasks):
    ...
    status = DailyStatus(date=yesterday, completed_at=datetime.now())
    test_db.add(status)
    test_db.commit()
    ...
    streak_info = streak_service.get_streak_info(test_db)
...
def test_today_incomplete_shows_previous_streak(test_db: Session):
