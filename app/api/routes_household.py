...
def list_household_tasks(
    frequency: Optional[str] = Query(None, pattern="^(weekly|biweekly|monthly|quarterly|annual|todo)$"),
    ...
    Args:
        frequency: Optional filter by frequency (weekly/biweekly/monthly/quarterly/annual/todo)
...
def create_household_task(
    ...
    """
    task = household_service.create_household_task(db, task_data)
    return task
...
def update_household_task(
    ...
    """
    task = household_service.update_household_task(db, task_id, task_data)
    if not task:
        raise HTTPException(status_code=404, detail="Household task not found")
    return task
