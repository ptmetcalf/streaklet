...
def get_today_info(db: Session = Depends(get_db)):
    """Get today's checklist with all tasks and completion status."""
    today = get_today()
...
def update_check(
    ...
    """Toggle a task check for a specific date."""
    task = task_service.get_task_by_id(db, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")

    check = check_service.update_task_check(db, check_date, task_id, check_update.checked)
    return {
        "date": check_date,
        "task_id": task_id,
